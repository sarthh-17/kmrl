...
// Generate the initial 10 Kochi Metro trainsets
const defaultFleet = Array.from({ length: 10 }, (_, i) => ({
  id: `Rake #${String(i + 1).padStart(2, '0')}`,
  mileage: Math.floor(Math.random() * 40000) + 15000, // Random baseline mileage
  safetyCleared: i % 3 !== 0, // Roughly 2/3rds start cleared
  jobCards: i % 4 === 0 ? Math.floor(Math.random() * 3) + 1 : 0, 
}));

export default function FleetDashboard() {
  // Initialize state with LocalStorage for fast persistence
  ...
  // Sync to LocalStorage whenever fleet data changes
  useEffect(() => {
  ...
  // Helper to determine status dynamically based on rules
  const getTrainStatus = (train) => {
    ...
    return { label: 'Ready', color: 'bg-[#00A896]/10 text-[#00A896] border-[#00A896]/30' };
  };
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[#0F172A] flex items-center gap-3">
            <Activity className="w-8 h-8 text-[#00A896]" />
              ...
              <div className="p-5 space-y-5">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                  ...
                  <button 
                    ...
                    className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none ${train.safetyCleared ? 'bg-[#00A896]' : 'bg-gray-300'}`}
                  >
